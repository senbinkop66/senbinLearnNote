...
我们经常性需要侦测响应式数据的变化，vue3中除了watch之外又出现了watchEffect
...
3. watch更底层，可以接收多种数据源，包括用于依赖收集的getter函数，**因此它完全可以实现watchEffect的功能**，同时由于可以指定getter函数，依赖可以控制的更精确，还能获取数据变化前后的值，因此如果需要这些时我们会使用watch。
4. `watchEffect`在使用时，传入的函数会立刻执行一次。`watch`默认情况下并不会执行回调函数，除非我们手动设置`immediate`选项。
5. 从实现上来说，`watchEffect(fn)`相当于`watch(fn,fn,{immediate:true})`
...
3. 两者均是用于构造响应式数据，但是`ref`主要解决**原始值**的响应式问题
