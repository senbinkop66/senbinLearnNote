...
- sessionStorage：仅在当前浏览器窗口关闭之前有效；关闭页面或浏览器后就会被清除。
- localStorage：始终有效，除非被手动清除。窗口或浏览器关闭也一直保存，因此用作持久数据；
- cookie：可设置失效时间，没有设置的话，默认是关闭浏览器后失效。**只在设置的cookie过期时间之前有效**，即使窗口关闭或浏览器关闭

**请求是否携带**

- cookie数据始终在**同源**的http请求中携带（即使不需要），**即cookie在浏览器和服务器间来回传递**。如果使用cookie保存过多数据会带来性能问题
...
垃圾回收对是否需要回收的问题**主要依赖于对变量的判定是否可访问**，由此衍生出两种主要的回收算法：
...
下面的方式就可以创建一个标识为`kop`的频道：

```js
const bc = new BroadcastChannel('kop');
```
...
了解了“共享存储+长轮询”这种模式。也许你会认为长轮询没有监听模式优雅，但实际上，有些时候使用“共享存储”的形式时，不一定要搭配长轮询。
...
“口口相传”的模式存在一个问题：如果页面不是通过在另一个页面内的`window.open`打开的（例如直接在地址栏输入，或从其他网站链接过来），这个联系就被打破了。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2019/3/31/169d468988a6ba8f~tplv-t2oaga2asx-image.image)
...
5. 预加载图片 有时候一个页面有很多图片，或者有几个很大的图片的时候，如果业务限制不考虑懒加载，也可以使用Web Worker来加载图片，可以参考一下[这篇文章的探索](https://juejin.cn/post/6844903510673211400)，这里简单提要一下。
...
页面的内容都是一帧一帧绘制出来的，浏览器刷新率代表浏览器一秒绘制多少帧。原则上说 1s 内绘制的帧数也多，画面表现就也细腻。目前浏览器大多是 60Hz（60帧/s），每一帧耗时也就是在 16.6ms 左右。那么在这一帧的（16.6ms） 过程中浏览器又干了些什么呢？



![img](E:\pogject\学习笔记\image\浏览器\浏览器一帧)
...
浏览器的渲染进程是**多线程的**，页面的渲染，JavaScript 的执行，事件的循环，都在这个进程内进行：
...
![img](E:\pogject\学习笔记\image\浏览器\482f33e0-8089-11eb-85f6-6fac77c0c9b3.png)
