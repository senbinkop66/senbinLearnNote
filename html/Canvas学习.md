...
- `data`
  ...
  [`Uint8ClampedArray`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Uint8ClampedArray)类型的一维数组，包含着 RGBA 格式的整型数据，范围在 0 至 255 之间（包括 255）。
...
## [创建一个`ImageData对象`](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#创建一个imagedata对象)
...
## [得到场景像素数据](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#得到场景像素数据)
...
### [**颜色选择器**](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#a_color_picker)
...
<body onload="draw();">
  <canvas id="test1" width="300" height="600"></canvas>
...
<script type="text/javascript">
  ...
  let hoverrdColor = document.getElementById("hovered-color");
...
## [在场景中写入像素数据](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#在场景中写入像素数据)
...
### [图片灰度和反相颜色](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#grayscaling_and_inverting_colors)
...
## [缩放和反锯齿](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#缩放和反锯齿)
...
## [保存图片](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#保存图片)
...
```js
  function saveImage() {
    let saveData = testCanvas.toDataURL("image/png");
    let dl = document.getElementById("download");
    dl.href = saveData;
    dl.download = 'filesave1'

    let event = new MouseEvent("click");
    dl.dispatchEvent(event);
    // let saveData = testCanvas.toDataURL("image/jpeg", 1);
  }
```
...
## [性能贴士](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#性能贴士)

下面是一些改善性能的建议

### [在离屏 canvas 上预渲染相似的图形或重复的对象](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#在离屏canvas上预渲染相似的图形或重复的对象)

如果发现自己在每个动画帧上重复了一些相同的绘制操作，请考虑将其分流到屏幕外的画布上。然后，您**可以根据需要频繁地将屏幕外图像渲染到主画布上，而不必首先重复生成该图像的步骤。**
...
### [避免浮点数的坐标点，用整数取而代之](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#避免浮点数的坐标点，用整数取而代之)
...
### [不要在用`drawImage`时缩放图像](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#不要在用drawimage时缩放图像)

在离屏 canvas 中缓存图片的不同尺寸，而不要用drawImage()去缩放它们。

### [使用多层画布去画一个复杂的场景](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#使用多层画布去画一个复杂的场景)
...
### [用 CSS 设置大的背景图](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#用css设置大的背景图)

如果像大多数游戏那样，你有一张静态的背景图，用一个静态的`<div>`元素，结合background 特性，以及将它置于画布元素之后。这么做可以避免在每一帧在画布上绘制大图。

### [用 CSS transforms 特性缩放画布](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#用css_transforms特性缩放画布)
...
### [关闭透明度](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#关闭透明度)
...
### [更多的贴士](https://developer.mozilla.org/zh-CN/docs/Web/API/Canvas_API/Tutorial/Optimizing_canvas#更多的贴士)
